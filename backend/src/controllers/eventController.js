...
class EventController {
  ...
  static async getPublicEventBySlug(req, res) {
    try {
      const { slug } = req.params;
      console.log('🔍 getPublicEventBySlug - slug:', slug);

      const event = await EventService.getPublicEventBySlug(slug);
      console.log('✅ getPublicEventBySlug - evento retornado:', JSON.stringify(event, null, 2));

      console.log('✅ getPublicEventBySlug - enviando resposta de sucesso');
      res.json({
        success: true,
        data: event
      });
    } catch (error) {
      console.error('❌ Erro ao buscar evento público por slug:', error);
  ...
  // Buscar evento para preview por slug
  static async getEventForPreviewBySlug(req, res) {
    try {
      const { slug } = req.params;
      console.log('🔍 getEventForPreviewBySlug - slug:', slug);

      const event = await EventService.getEventForPreviewBySlug(slug);
      console.log('✅ getEventForPreviewBySlug - evento retornado:', JSON.stringify(event, null, 2));

      if (!event) {
        console.log('❌ getEventForPreviewBySlug - evento não encontrado');
        return res.status(404).json({
          success: false,
          message: 'Evento não encontrado'
        });
      ...
      console.log('✅ getEventForPreviewBySlug - enviando resposta de sucesso');
      res.json({
        success: true,
        data: event
      });
    } catch (error) {
      console.error('❌ Erro ao buscar evento para preview por slug:', error);
  ...
  // Buscar configuração do formulário público por slug
  static async getPublicFormConfigBySlug(req, res) {
    try {
      const { slug } = req.params;
      ...
      console.error('❌ Erro ao buscar configuração do formulário público por slug:', error);
  ...
  // Buscar configuração da página pública para preview por slug (sem restrições)
  static async getPublicPageConfigForPreviewBySlug(req, res) {
    try {
      const { slug } = req.params;
      console.log('🔍 getPublicPageConfigForPreviewBySlug - slug:', slug);

      const event = await EventService.getEventForPreviewBySlug(slug);
      const config = await EventService.getPublicPageConfigForPreview(event.id);
      console.log('✅ getPublicPageConfigForPreviewBySlug - config retornado:', JSON.stringify(config, null, 2));
      ...
      console.error('❌ Erro ao buscar configuração da página pública para preview por slug:', error);
